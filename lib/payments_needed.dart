...
List<PaymentData> paymentsNeeded(List<Member> members){
  ...
  if (members.where((member) => member.balance != 0).length>0){
    ...
    do
    {
      ...
      {
        transactions.add(new PaymentData(payerId: minPerson.memberId, payerUsername: minPerson.username, payerNickname: minPerson.nickname, takerId: maxPerson.memberId, takerUsername: maxPerson.username, takerNickname: maxPerson.nickname, amount: minPerson.balance.abs()));
        maxPerson.balance -= minPerson.balance.abs();
        minPerson.balance = 0;
      }
      else
      {
        transactions.add(new PaymentData(payerId: minPerson.memberId, payerUsername: minPerson.username, payerNickname: minPerson.nickname, takerId: maxPerson.memberId, takerUsername: maxPerson.username, takerNickname: maxPerson.nickname, amount: maxPerson.balance.abs()));
